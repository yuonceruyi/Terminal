...
namespace YuanTu.Core.Services.PrintService
{
    public class BarCode
    {
        public class Code128
        {
            ...
            private readonly DataTable m_Code128 = new DataTable();
                ...
                switch (p_Code)
                {
                    case Encode.Code128C:
                        ...
                        while (p_Text.Length != 0)
                        {
                            ...
                            try
                            {
                                var _CodeNumb128 = int.Parse(p_Text.Substring(0, 2));
                            }
                            catch
                            {
                    ...
                    case Encode.EAN128:
                        ...
                        break;
                    default:
                        _Examine = p_Code == Encode.Code128A ? 103 : 104;

                        while (p_Text.Length != 0)
                        {
                            var _Temp = 0;
                            var _ValueCode = GetValue(p_Code, p_Text.Substring(0, 1), ref _Temp);
                            if (_ValueCode.Length == 0) throw new Exception("无效的字符集!" + p_Text.Substring(0, 1));
                            _Text += _ValueCode;
                            _TextNumb.Add(_Temp);
            ...
            private string GetValue(Encode p_Code, string p_Value, ref int p_SetID)
            {
                ...
                var _Row = m_Code128.Select(p_Code + "='" + p_Value + "'");
                if (_Row.Length != 1) throw new Exception("错误的编码" + p_Value);
            ...
            {
                var _Row = m_Code128.Select("ID='" + p_CodeId + "'");
                if (_Row.Length != 1) throw new Exception("验效位的编码错误" + p_CodeId);
                return _Row[0]["BandCode"].ToString();
            }
            ...
            {
                var _Value = p_Text.ToCharArray();
                var _Width = 0;
                for (var i = 0; i != _Value.Length; i++)
                {
                    _Width += int.Parse(_Value[i].ToString())*(Magnify + 1);
                }
                var _CodeImage = new Bitmap(_Width, (int) Height);
                var _Garphics = Graphics.FromImage(_CodeImage);
                var _LenEx = 0;
                for (var i = 0; i != _Value.Length; i++)
                {
                    var _ValueNumb = int.Parse(_Value[i].ToString())*(Magnify + 1); //获取宽和放大系数
                    _Garphics.FillRectangle((i & 1) != 0 ? Brushes.White : Brushes.Black,
                        new Rectangle(_LenEx, 0, _ValueNumb, (int) Height));
                    _LenEx += _ValueNumb;
                }
                _Garphics.Dispose();
                return _CodeImage;
            ...
            private void GetViewText(Bitmap p_Bitmap, string p_ViewText)
            {
                ...
                var _Graphics = Graphics.FromImage(p_Bitmap);
                var _DrawSize = _Graphics.MeasureString(p_ViewText, ValueFont);
                ...
                var _StarY = p_Bitmap.Height - (int) _DrawSize.Height;
                _Graphics.FillRectangle(Brushes.White, new Rectangle(0, _StarY, p_Bitmap.Width, (int) _DrawSize.Height));
                _Graphics.DrawString(p_ViewText, ValueFont, Brushes.Black, 0, _StarY);
            }
