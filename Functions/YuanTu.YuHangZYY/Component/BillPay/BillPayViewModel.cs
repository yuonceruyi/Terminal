...
using YuanTu.YuHangZYY.Component.BillPay.Models;
using YuanTu.YuHangZYY.NativeService;
using YuanTu.YuHangZYY.NativeService.Dto;

namespace YuanTu.YuHangZYY.Component.BillPay
{
    public class BillRecordViewModel :Default.Component.BillPay.ViewModels.BillRecordViewModel
    {
        ...
        protected override Queue<IPrintable> BillPayPrintables()
        {
          ...
          //  sb.Append($"交易类型：自助缴费\n");
            ...
            {
                var citizenBlance = decimal.Parse(PrePayModel.Checkout?.CitizenCardBalance ?? "0") * 100;
                if (PaymentModel.PayMethod == PayMethod.智慧医疗)
                {
                    sb.AppendLine($"智慧医疗余额：{(citizenBlance - PaymentModel.Self).In元()}");
                }
                else
                {
                    sb.AppendLine($"智慧医疗余额：{citizenBlance.In元()}");
                }
            }
            sb.Append($"收费项目明细：\n");
