...
namespace YuanTu.YuHangZYY.NativeService
{
    /// <summary>
    /// 联众HIS的本地化调用接口
    /// </summary>
    public static class LianZhongHisService
    {
        ...
        public static Result<RegisterPay> ExcuteHospitalRegister(RegisterPayRequest request)
        {
            ...
            if (!outPut.IsSuccess)
            {
                return Result<RegisterPay>.Fail(outPut.ResultCode, outPut.Message);
            }
            var outArr = outPut.Value.Split(new[] { '#', '|' });
            if (outArr.Length <= 1)
            {
                return Result<RegisterPay>.Fail(outPut.ResultCode, "His 返回参数不正确");
            }
            if (outPut.Value.StartsWith("-"))//负数结尾都是错
            {
                return Result<RegisterPay>.Fail(outPut.ResultCode, outArr.Last());
            }
        ...
        public static Result<Checkout> ExcuteHospitalCheckout(CheckoutRequest request)
        {
            ...
            if (!outPut.IsSuccess)
            {
                return Result<Checkout>.Fail(outPut.ResultCode,outPut.Message);
            }
            var outArr = outPut.Value.Split(new[] { '#', '|' });
            if (outArr.Length <= 1)
            {
                return Result<Checkout>.Fail(outPut.ResultCode, "His 返回参数不正确");
            }
            if (outPut.Value.StartsWith("-"))
            {
                return Result<Checkout>.Fail(outPut.ResultCode, outArr.Last());
            }
            var aSpit = outPut.Value.Split('#');
            var bSpit = aSpit[2].Split('|');
            var cSpit = aSpit[3].Split('|');
        ...
        public static Result<GetTicketCheckout> ExcuteHospitalGetTicketCheckout(GetTicketCheckoutRequest request)
        {
            ...
            if (!outPut.IsSuccess)
            {
                return Result<GetTicketCheckout>.Fail(outPut.ResultCode, outPut.Message);
            }
            var outArr = outPut.Value.Split(new[] { '#', '|' });
            if (outArr.Length <= 1)
            {
                return Result<GetTicketCheckout>.Fail(outPut.ResultCode, "His 返回参数不正确");
            }
            if (outPut.Value.StartsWith("-"))
            {
                return Result<GetTicketCheckout>.Fail(outPut.ResultCode, outArr.Last());
            }
            if (outArr.Length < 11)
            {
                return Result<GetTicketCheckout>.Fail(outPut.ResultCode, "His 返回参数不正确(长度不够)");
            }
        ...
        private static Result<string> InternalRunExeInvoke(string input)
        {
            ...
            lock (LockObj)
            {
                try
                {
                    ...
                    Logger.Net.Info($"[本地HIS]地址:{HisExePath},入参:{input}");
                    ...
                    Logger.Net.Info($"出参:{outPut[0]}  耗时:{timeelsp}ms");
                    ...
                    else
                    {
                       ...
                       // var sbuilder = new StringBuilder("进程杀死记录");
                        ...
                        return Result<string>.Fail(-100,"调用本地HIS服务异常，请稍后再试");
                    }
                }
                catch (Exception ex)
                {
                    Logger.Net.Error($"调用本地HIS服务异常 {ex.Message} {ex.StackTrace}");
                    return Result<string>.Fail(-100,$"调用本地HIS服务异常 {ex.Message}");
                }
