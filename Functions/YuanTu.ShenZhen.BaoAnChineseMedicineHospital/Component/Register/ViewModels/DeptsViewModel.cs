        ...
        protected override void Confirm(Info i)
        {
            ...
            if (!RegTypesModel.SelectRegType.SearchDoctor) //普通
            {
                DoCommand(lp =>
                {
                    ...
                    {
                        #region 号码筛选   2017年9月5日修改，所有科室加普通号、康复科、皮肤科、骨伤科三区、妇科   加急诊号

                        if (!(DeptartmentModel.所选科室.deptName.Contains("康复科") || DeptartmentModel.所选科室.deptName.Contains("皮肤科") || DeptartmentModel.所选科室.deptName.Contains("骨伤科三区") || DeptartmentModel.所选科室.deptName.Contains("妇科")))
                        {
                            ////去掉普通号、急诊
                            Res排班信息查询.data = Res排班信息查询.data?.Where(d => !d.doctName.Contains("急诊")).ToList();
                        }
                        #endregion
