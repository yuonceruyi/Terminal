            ...
            try
            {
                ...
                while (_working)
                {
                    ...
                    if (pos.IsSuccess && pos.Value == CardPos.停卡位)
                    {
                        ...
                        if (rest.IsSuccess)
                        {
                            Logger.Main.Info($"[宝安中医院A6读卡序号成功][返回原始数据]{string.Join(" ", rest.Value)}");
                            var trackNo = BitConverter.ToUInt32(rest.Value, 0).ToString();  //读到有卡
                            Logger.Main.Info($"[读取卡序号成功][trackNo]{trackNo}");
                        }
                        ...
                        if (rest.IsSuccess)
                        {
                            Logger.Main.Info($"[宝安中医院A6读卡号成功][返回原始数据]{string.Join(" ", rest.Value)}");
                        ...
                        if (rest.IsSuccess)
                        {
                            Logger.Main.Info($"[宝安中医院A6读验证码成功][返回原始数据]{string.Join(" ", rest.Value)}");
        ...
        protected override void OnGetInfo(string cardNo, string extendInfo = null)
        {
            ...
            DoCommand(ctx =>
            {
                ...
                if (PatientModel.Res病人信息查询.success)
                {
                    ...
                    if (string.IsNullOrWhiteSpace(PatientModel.当前病人信息.idNo) && string.IsNullOrWhiteSpace(PatientModel.当前病人信息.guardianNo))
                    {
