...
using RegisterModel = YuanTu.YiWuBeiYuan.Component.Register.Models.RegisterModel;
    ...
    public class ScheduleViewModel:YuanTu.Default.Component.Register.ViewModels.ScheduleViewModel
    {
                ...
                DoCommand(lp =>
                {
                    try
                    {
                        ...
                        else
                        {
                            ...
                            PaymentModel.Self = decimal.Parse(info.billFee);
                            PaymentModel.Insurance = 0;
                            PaymentModel.Total = decimal.Parse(info.billFee);
                            PaymentModel.NoPay = PaymentModel.Self == 0;
                            PaymentModel.ConfirmAction = Confirm;
                            var schedulInfo = ScheduleModel.所选排班;
                            PaymentModel.LeftList = new List<PayInfoItem>()
        ...
        private Result RegisterConfirm(LoadingProcesser lp)
        {
            ...
            if (CardModel.CanUseInsurance())
            {
                ...
                var ratiostr = string.Join("||", rm.自负比例列表?.Select(p => p.ToString())??new string[0]);
        ...
        protected override Queue<IPrintable> AppointPrintables()
        {
            ...
            sb.Append($"交易时间：{DateTimeCore.Now.ToString("yyyy-MM-dd HH:mm:ss")}\n");
            sb.Append($"柜员编号：{FrameworkConst.OperatorId}\n");
            if (RegTypesModel.SelectRegType.SearchDoctor)//选择医生模式
            {
                sb.Append($"请于就诊当天提前半小时取号就诊。\n");
            }
            else
            {
