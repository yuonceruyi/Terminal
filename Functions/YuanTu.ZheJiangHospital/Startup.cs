...
namespace YuanTu.ZheJiangHospital
{
    public class Startup : DefaultStartup
    {
        ...
        public override bool RegisterTypes(ViewCollection children)
        {
            ...
            children.Add(A.YY.Dept, "选择预约科室", typeof(Component.Appoint.Views.DeptsView), A.YY.Schedule);
            children.Add(A.YY.Schedule, "选择医生排班", typeof(Component.Appoint.Views.ScheduleView), A.YY.Time);
            children.Add(A.YY.Time, "选择排班分时", typeof(Component.Appoint.Views.SourceView), A.YY.Confirm);
            ...
            children.Add(A.JF.BillRecord, "待缴费信息", typeof(Component.BillPay.Views.BillRecordView), A.JF.Confirm);
        ...
        public override void AfterStartup()
        {
            ...
            var config = serviceLocator.GetInstance<IConfigurationManager>();

            DataHandler.ConnectionString = config.GetValue("浙江医院:ConnectionString");
            DataHandler.ExePath = config.GetValue("浙江医院:ExePath");

            PConnection.HisCode = config.GetValue("浙江医院:ICBC:HisCode");

            var ip = config.GetValue("浙江医院:ICBC:IP");
            var port = config.GetValue("浙江医院:ICBC:Port");

            PConnection.ICBC = new IPEndPoint(IPAddress.Parse(ip), Convert.ToInt32(port));

            ReporterDataHandler.Disabled = true;
            FrameworkConst.HospitalName = "浙江医院";

            var eventAggrator = serviceLocator.GetInstance<IEventAggregator>();
            var pubSubEvent = eventAggrator.GetEvent<PubSubEvent<DateTime>>();
            pubSubEvent.Subscribe(BalanceDeal.Check, ThreadOption.BackgroundThread);
        }
        ...
        {
            return new[] { "CurrentResource\\浙江医院.xml" };
        }
    }
}