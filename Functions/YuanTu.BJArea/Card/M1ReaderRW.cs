...
namespace YuanTu.BJArea.Card
{
    ...
    public class M1ReaderRW: M1
    {
        ...
        /// </summary>        
        public static void WriteCommChunk0(string CardNo, string HosNo, EnumM1Valid eValid)
        {
            ...
            Result result = rfCardReader.WirteBlock(4, 0, false, KeyValue, data);          
        }
        /// <summary>
        /// //第4扇区 块0 平台卡号	院区代码	启用标志
        /// </summary>        
        public static void ReadCommChunk0(out string CardNo, out string HosNo, out string valid)
        {
            ...
            valid = string.Empty;
            byte[] data;
            Result<byte[]> result = rfCardReader.ReadBlock(4, 0, false, KeyValue);            
            if (!result.IsSuccess)
            {
                return;
            }
        ...
        /// </summary>        
        public static void WriteCommChunk1(string IssueDate, string validDate, string BeginDate)
        {
            byte[] chunk0 = new byte[15];
            ...
            data[15] = Checkcode(chunk0);
            //第4扇区 块1	发行日期	有效日期	启用日期	
            Result result = rfCardReader.WirteBlock(4, 1, false, KeyValue, data);         
        }
        ...
        public static void ReadCommChunk1(out string IssueDate, out string validDate, out string BeginDate)
        {
            ...
            byte[] data;
            Result<byte[]> result = rfCardReader.ReadBlock(4, 1, false, KeyValue);
            if (!result.IsSuccess)
            {
                return;
            }
        ...
        /// 第4扇区 块2	平台患者流水号		
        /// </summary>        
        public static void WriteCommChunk2(string patientId)
        {
            byte[] chunk0 = new byte[15];
            ...
            data[15] = Checkcode(chunk0);
            //第4扇区 块2	平台患者流水号	  
            Result result = rfCardReader.WirteBlock(4, 2, false, KeyValue, data);            
        }
        /// <summary>
        /// 第4扇区 块2	平台患者流水号		
        /// </summary>        
        public static void ReadCommChunk2(out string patientId)
        {
            ...
            byte[] data ;
            Result<byte[]> result = rfCardReader.ReadBlock(4, 2, false, KeyValue);
            if (!result.IsSuccess)
            {
                return;
            }
        ...
        public static void WritePatientChunk0(string name, string sex)
        {
            byte[] chunk0 = new byte[15];
            ...
            data[15] = Checkcode(chunk0);
            //第5扇区 块0 姓名、性别   
            Result result = rfCardReader.WirteBlock(5, 0, false, KeyValue, data);         
        }
        /// <summary>
        /// //第5扇区 块0 姓名、性别
        /// </summary>
        ...
        public static void ReadPatientChunk0(out string name, out string sex)
        {
            ...
            sex = string.Empty;            
            byte[] data;
            Result<byte[]> result = rfCardReader.ReadBlock(5, 0, false, KeyValue);
            if (!result.IsSuccess)
            {
                return;
            }
        ...
        /// </summary>       
        public static void WritePatientChunk1(EnumM1IdType enumM1IdType, string idNo, string phone)
        {
            ...
            //第5扇区 块1
            Result result = rfCardReader.WirteBlock(5, 1, false, KeyValue, chunk1);
            //块2
            result = rfCardReader.WirteBlock(5, 2, false, KeyValue, data2);            
        }
        /// <summary>
        /// //第5扇区 块1和2 证件类型、证件号、联系方式
        /// </summary>       
        public static void ReadPatientChunk1(out string idType, out string idNo, out string phone)
        {
            ...
            phone = string.Empty;
            byte[] data2, chunk1;
            Result<byte[]> result = rfCardReader.ReadBlock(5, 1, false, KeyValue);
            if (!result.IsSuccess)
            {
                return;
            }
            chunk1 = result.Value;
            result = rfCardReader.ReadBlock(5, 2, false, KeyValue);
            if (!result.IsSuccess)
            {
                return;
            }
