...
namespace YuanTu.BJArea.Services.PrintService
{
    public class PrintHelper
    {
        ...
        public Result Print()
        {
            try
            {
                ...
                if (FrameworkConst.Strategies[0] == DeviceType.Clinic)
                    docToPrint.DefaultPageSettings.PaperSize = GetPageSize(46.5);
            ...
            catch (Exception ex)
            {
                ...
                Logger.Printer.Info($"[{printerName}]打印失败 \r\n {ex.Message}\r\n{ex.StackTrace}");
        ...
        private void docToPrint_PrintPage(object sender, PrintPageEventArgs e)
        {
            ...
            {
                if (top + printQueue.Peek().GetHeight(e.Graphics, e.PageBounds.Width) > e.PageBounds.Height)
                    break;
