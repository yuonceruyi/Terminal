...
namespace YuanTu.ShenZhen.BaoAnCenterHospital.Component.InfoQuery.ViewModels
{
    public class DiagReportViewModel : YuanTu.Default.Component.InfoQuery.ViewModels.DiagReportViewModel
    {
        ...
        //        DoCommand(tt =>
        //        {
        //            tt.ChangeText("正在请求检验报告打印,请稍候……");
        //            TimeOut =8;
        //            //var txt = "@echo Start Call;\n\"C:\\Program Files\\hope\\HopeTouchClient\\Lis.Client.TouchPrintReport.exe\" " + PatientModel.当前病人信息.patientId+ ";\n@echo End Call;\nPAUSE;";
        //            var txt = "\"D:\\hope\\HopeTouchClient\\Lis.Client.TouchPrintReport.exe\" " + PatientModel.当前病人信息.patientId;
        ...
        public override void OnEntered(NavigationContext navigationContext)
        {
            try
            {
                TimeOut =ConfigBaoAnCenterHospital.LisTimeOut - 2;
                DoCommand(tt =>
                {
                    tt.ChangeText("请耐心等待界面提示您的报告信息，并根据提示取走您所有的报告……");
                    var fileName = ConfigBaoAnCenterHospital.LisFolderPath + "Lis.Client.TouchPrintReport.exe";
                    var da = new Process
                    {
                    ...
                    da.Start();
                    System.Threading.Thread.Sleep(ConfigBaoAnCenterHospital.LisTimeOut * 1000);
                }, false);
                return;
            }
            catch (Exception ex)
