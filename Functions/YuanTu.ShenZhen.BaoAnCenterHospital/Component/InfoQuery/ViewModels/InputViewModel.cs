            ...
            DoCommand(ctx =>
            {
                ...
                if (QueryChoiceModel.InfoQueryType == InfoQueryTypeEnum.药品查询)
                {
                    ...
                    if (MedicineModel.Res药品项目查询.success)
                    {
                        ...
                        for (int i = 0; i < MedicineModel.Res药品项目查询.data.Count; i++)
                        {
                            if (!string.IsNullOrEmpty(MedicineModel.Res药品项目查询.data[i].price))
                                MedicineModel.Res药品项目查询.data[i].price = (Convert.ToDecimal(MedicineModel.Res药品项目查询.data[i].price) * 100).ToString().Split('.')[0];
                        }
                        Next();
                    }
                    else
