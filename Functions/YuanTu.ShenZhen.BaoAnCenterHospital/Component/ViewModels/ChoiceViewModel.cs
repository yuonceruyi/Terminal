...
namespace YuanTu.ShenZhen.BaoAnCenterHospital.Component.ViewModels
{
    public class ChoiceViewModel : YuanTu.Default.Component.ViewModels.ChoiceViewModel
    {
        protected override void Do(ChoiceButtonInfo param)
        {
            ...
            #region 打印机状态检测
            if (!ConfigBaoAnCenterHospital.ClosePrintStatusCheck)
            {
            ...
            switch (param.ButtonBusiness)
            {
                ...
                case Business.挂号:
                    ...
                    //engine.JumpAfterFlow(new FormContext(A.ChaKa_Context, A.CK.Choice), RegisterJump, new FormContext(A.XianChang_Context, A.XC.Dept), param.Name);
                    //科室分类
                    engine.JumpAfterFlow(new FormContext(A.ChaKa_Context, A.CK.Choice), RegisterJump, new FormContext(A.XianChang_Context, InnerA.XC.DeptType), param.Name);
                ...
                case Business.预约:
                    ...
                    break;
                case Business.取号:
                    engine.JumpAfterFlow(new FormContext(A.ChaKa_Context, A.CK.Choice), TakeNumJump, new FormContext(A.QuHao_Context, A.QH.Record), param.Name);
                    break;
                case Business.缴费:
                ...
                case Business.住院押金:
                    ...
                    engine.JumpAfterFlow(new FormContext(A.ZhuYuan_Context, A.ZY.Choice), IpRechargeJump, new FormContext(A.IpRecharge_Context, A.ZYCZ.RechargeWay), param.Name);
                    break;
                case Business.检验结果:
                    var queryChoiceModel = GetInstance<IQueryChoiceModel>();
                    engine.JumpAfterFlow(new FormContext(A.ChaKa_Context, A.CK.Choice), CreateJump, new FormContext(A.DiagReportQuery, A.JYJL.DiagReport), param.Name);
                    break;
                case Business.住院一日清单:
                    choiceModel.AuthContext = A.ZhuYuan_Context;
                    engine.JumpAfterFlow(new FormContext(A.ZhuYuan_Context, A.ZY.Choice), CreateJump, new FormContext(A.InDayDetailList_Context, A.ZYYRQD.Date), param.Name);
                    break;
                default:
        ...
        protected override Task<Result<FormContext>> RegisterJump()
        {
            return DoCommand(lp =>
            {
                ...
                var regtypesModel = GetInstance<IRegTypesModel>();
                var deptartmentModel = GetInstance<IDeptartmentModel>();
                var choiceModel = GetInstance<IChoiceModel>();

                regdateModel.RegDate = DateTimeCore.Now.ToString("yyyy-MM-dd");

                regtypesModel.SelectRegType = null;
                regtypesModel.SelectRegTypeName = "";
                ...
                if (deptartmentModel.Res排班科室信息查询?.success ?? false)
                {
                    if (deptartmentModel.Res排班科室信息查询?.data?.Count > 0)
                    {
                        ...
                        //下午下班时间（5点）提前10分钟调至只允许挂急诊号
                        if (DateTimeCore.Now > (DateTimeCore.Now.Date.AddHours(16).AddMinutes(50)))
                        {
