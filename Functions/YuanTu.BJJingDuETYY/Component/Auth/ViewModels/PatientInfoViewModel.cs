...
#pragma warning restore 612
        ...
        public override void Confirm()
        {
            ...
            if (Phone.IsNullOrWhiteSpace() || (IdNo.IsNullOrWhiteSpace() && GuardIdNo.IsNullOrWhiteSpace()))
            {
                switch (ChoiceModel.Business)
                {
                    case Business.建档:
                    case Business.预约:
                        ShowAlert(false, "信息不全", "您未登记电话号码或身份证号，请您到就近服务台完善个人信息，再进行预约。");
                        return;
                    case Business.取号:
                        ShowAlert(false, "信息不全", "您未登记电话号码或身份证号，请您到就近服务台完善个人信息，再进行取号。");
                        return;
                    case Business.检验结果:
                        break;
                    default:
                        ShowAlert(false, "信息不全", "您未登记电话号码或身份证号，请您就诊完毕后，去服务台完善个人信息。");
                        break;
                }
            }
