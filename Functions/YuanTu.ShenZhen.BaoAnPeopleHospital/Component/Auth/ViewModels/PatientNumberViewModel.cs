...
{
    public class PatientNumberViewModel : ViewModelBase
    {
        public PatientNumberViewModel()
        {
        ...
        public virtual void Confirm(TextBox txtPatientId)
        {
            var sPatientId = txtPatientId.Text;
            ...
            if (sPatientId.EndsWith("\n"))
            {
                 ...
                 * 挂号单二维码格式
                 * 
                 * https://wxis.91160.com/wxis/anaCode.do?c=0000038961&d=&h=&u=113&s=391cebca0a&n=陈杏光
                 * 
                 * 
                */
                string patientId = "";

                if (sPatientId.StartsWith("https://"))
                {
                    patientId = sPatientId.Substring(sPatientId.IndexOf("c=") + 2, 10);
                    Logger.Main.Info($"扫描挂号单条码进入，条码内容为“{sPatientId}”；获取到的登记号为“{patientId}”");
                }
                else
                {
                    patientId = sPatientId.Replace("\n", "").Replace("\r", "");
                }
                OnGetInfo(patientId.PadLeft(10, '0'));
                txtPatientId.Text = "";
                txtPatientId.Focus();
            }
            return;
