...
using System.Linq;
using System.Text;
using YuanTu.TongXiangHospitals.ISO8583.CPUCard;
...
namespace YuanTu.TongXiangHospitals.ISO8583
{
    public class Encoder
    {
        ...
        public List<TlvPackageData> Packages_58 { get; set; }

        public List<CPUTlv> ICPackages { get; set; } 

        public Config Config { get; set; } = Config.DefaultConfig;

        public static BuildConfig BuildConfig { get; set; } = BuildConfig.DefaultConfig;
        ...
        public byte[] Encode()
        {
            ...
            foreach (var value in Values.OrderBy(kvp=>kvp.Key))
                if (value.Key == 48 && string.IsNullOrEmpty(value.Value))
                    ...
                    IC(sb);
                else if (value.Key == 58 && string.IsNullOrEmpty(value.Value))
                    SafeData(sb);
                else
        ...
        public static void SingleTlv(StringBuilder sb, TlvPackageData data,string tagformat="X")
        {
            var f = BuildConfig.Packages[data.Tag];
           
            sb.Append(data.Tag.ToString(tagformat));
            if (!f.OmitLength)
        ...
        private void SafeData(StringBuilder sb)
        {
            var sbuilder = new StringBuilder();
            foreach (var data in Packages_58)
            {
                SingleTlv(sbuilder, data,"X2");
            }
            var realLen = sbuilder.Length/2;
            //if (realLen % 2!=0)
            //{
            //    sbuilder.Append("00");
            //    realLen += 1;
            //}
            sb.Append(realLen.ToString("D4"));
            sb.Append(sbuilder);
