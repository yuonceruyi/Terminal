...
namespace YuanTu.TongXiangHospitals.HealthInsurance.Service
{
    public class SiService : ISiService
    {
        ...
        public Result Initialize()
        {
            ...
            Logger.Net.Debug($"[{Prefix}] [{index}] f_UserBargaingInit，开始初始化医保接口");
            ...
            Logger.Net.Debug($"[{Prefix}] [{index}] [耗时：{time}] f_UserBargaingInit，初始化医保接口结果{SiConfig.InitializeSuccess}");
        ...
        public Result OperatorSignIn(int transNo, string req)
        {
            ...
            var taskSuccess = task.Wait(60000);
            if(!taskSuccess)
                return Result.Fail("医保操作员签到超时");
            SiModel.RetMessage = sb.ToString();
        ...
        private Result SiApply(int transNo, string req)
        {
            ...
            {
                sb.Append(FakerSiServer(transNo));
                Logger.Net.Debug($"[{Prefix}] [{index}] [res{SiTransName[transNo]}] [耗时：0]\n{sb}");
            }
            else
            {
                var watch = Stopwatch.StartNew();
                ...
                {
                    SiModel.Ret =
                        UnSafeMethods.f_UserBargaingApply(transNo, SiConfig.TransSeq, req, sb, SiConfig.HospitalCode);
