namespace YuanTu.ChongQingArea.SiHandler
{
    ...
    /// <summary>
    ///     功能描述: 将指定住院(或门诊)号对应的费用进行结算，并产生结算记录
    /// </summary>
    public class Req结算 : Req
    {
        public override string 交易类别代码 => "05";
        public override string 交易类别 => "结算";

        public override string ToQuery()
        {
            return
                $"{交易类别代码}|{住院号_门诊号}|{结算类别}|{住院床日}|{经办人}|{账户余额支付标志}|{中途结算终止日期}|{本次结算总金额}|{本次结算明细总条数}|{险种类别}|{HIS开发商编码}|{工伤认定编号}|{工伤认定疾病编码}|{尘肺结算类型}|{出院科室编码}|{出院医师编码}";
        }
        ...
        ///     Varchar2(3)
        ///     非空
        ///     0	正常结算 10	中途结算 20	出院转家庭病床结算 30	IC 卡挂失结算 40	精神病请假开始中途结算 50	精神病请假结束中途结算
        /// </summary>
        public string 结算类别 { get; set; }
        ...
        ///     中途结算终止日期
        ///     DATE
        ///     格式：YYYY-MM-DD 非中途结算时，字段可以为空
        /// </summary>
        public string 中途结算终止日期 { get; set; }
        ...
        ///     Varchar2(5)
        ///     非空
        ///     HIS 开发商的系统编码，详见附件开发商列 表
        /// </summary>
        public string HIS开发商编码 { get; set; }
        ...
        ///     尘肺结算类型
        ///     Varchar2(3)
        ///     0、尘肺单病种结算 1、尘肺项目结算 工伤住院时，看尘肺不为空
        /// </summary>
    ...
    ///     功能描述: 将指定住院(或门诊)号对应的费用进行结算，并产生结算记录
    /// </summary>
    public class Res结算 : Res
    {
        public string ToQuery()
        {
            return
                $"{执行代码}|{交易流水号}|{统筹支付}|{帐户支付}|{公务员补助}|{现金支付}|{大额理赔金额}|{历史起付线公务员返还}|{帐户余额}|{单病种定点医疗机构垫支}|{民政救助金额}|{民政救助门诊余额}|{耐多药项目支付金额}|{一般诊疗支付数}|{神华救助基金支付数}|{本年统筹支付累计}|{本年大额支付累计}|{特病起付线支付累计}|{耐多药项目累计}|{本年民政救助住院支付累计}|{中心结算时间}|{本次起付线支付金额}|{本次进入医保范围费用}|{药事服务支付数}|{医院超标扣款金额}|{生育基金支付}|{生育现金支付}|{工伤基金支付}|{工伤现金支付}|{工伤单病种机构垫支}|{工伤全自费原因}";
        }

        public static Res结算 Parse(string s)
        {
            var list = s.Split('|');
            var res = new Res结算();
            res.执行代码 = list[0];
            ...
            res.现金支付 = list[5];
            res.大额理赔金额 = list[6];
            res.历史起付线公务员返还 = list[7];
            res.帐户余额 = list[8];
            res.单病种定点医疗机构垫支 = list[9];
            res.民政救助金额 = list[10];
            res.民政救助门诊余额 = list[11];
            res.耐多药项目支付金额 = list[12];
            res.一般诊疗支付数 = list[13];
            res.神华救助基金支付数 = list[14];
            res.本年统筹支付累计 = list[15];
            res.本年大额支付累计 = list[16];
            res.特病起付线支付累计 = list[17];
            res.耐多药项目累计 = list[18];
            res.本年民政救助住院支付累计 = list[19];
            res.中心结算时间 = list[20];
            res.本次起付线支付金额 = list[21];
            res.本次进入医保范围费用 = list[22];
            res.药事服务支付数 = list[23];
            res.医院超标扣款金额 = list[24];
            res.生育基金支付 = list[25];
            res.生育现金支付 = list[26];
            res.工伤基金支付 = list[27];
            res.工伤现金支付 = list[28];
            res.工伤单病种机构垫支 = list[29];
            res.工伤全自费原因 = list[30];
            // 执行代码|交易流水号|统筹支付|帐户支付|公务员补助|现金支付|大额理赔金额|历史起付线公务员返还|帐户余额|单病种定点医疗机构垫支|民政救助金额|民政救助门诊余额|耐多药项目支付金额|一般诊疗支付数|神华救助基金支付数|本年统筹支付累计|本年大额支付累计|特病起付线支付累计|耐多药项目累计|本年民政救助住院支付累计|中心结算时间|本次起付线支付金额|本次进入医保范围费用|药事服务支付数|医院超标扣款金额|生育基金支付|生育现金支付|工伤基金支付|工伤现金支付|工伤单病种机构垫支|工伤全自费原因
            return res;
        }
