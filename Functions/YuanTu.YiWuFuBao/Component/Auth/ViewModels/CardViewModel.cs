...
using YuanTu.YiWuFuBao.Component.Auth.Dialog;
...
namespace YuanTu.YiWuFuBao.Component.Auth.ViewModels
{
    public class CardViewModel : YuanTu.Default.Component.Auth.ViewModels.CardViewModel
    {
        ...
        public Visibility ShowOldMachine => ConstInner.IsOldMachine ? Visibility.Visible : Visibility.Hidden;
        public Visibility ShowNewMachine => ConstInner.IsOldMachine ? Visibility.Hidden : Visibility.Visible;

        public Uri SiCardReaderUrl { get; set; }
        ...
        protected ConfirmExModel confirmExModel=null;

        private bool cardInReader = false;
        public override void OnSet()
        {
            ...
            {
                BackUri = ResourceEngine.GetImageResourceUri("读卡器_社保读卡器");
            }
            else
            {
        ...
        public override void OnEntered(NavigationContext navigationContext)
        {
            ...
            if (ConstInner.IsOldMachine)
            {
                ...
                {
                    ShowSiCardAnimation = Visibility.Collapsed;
        ...
        protected override void StartRead()
        {
            ...
            Task.Run(() =>
            {
                try
                {
                    SipayHandler.Uninit();//强制反初始化
                    HospitalInputFocus = false;
                    HospitalInputFocus = true;
                    cardInReader = false;//读卡器无卡，允许从读卡器、扫码枪得到输入
                    if (ConstInner.IsOldMachine)
                    {
                        //老机器社保卡
                        if (CardModel.CardType==CardType.社保卡)
                        {
                            ...
                            try
                            {
                                ...
                                YiWuNativeProvider.dv_beep(handle, 100); //蜂鸣
                                while (_working)
                                {


                                    var no = new StringBuilder(1024);
                                    YiWuNativeProvider.readcardid(handle, '1', no);

                                    //var rest = YiWuNativeProvider.Read();
                                    if (no.ToString().Length > 0)
                                    {
                                        //  var no = rest.Value.CardNumber;
                                        Logger.Main.Info($"[社保读卡]社保卡号:{no}");
                                        StartReadInsuranceCard(no.ToString());
                                        break;
                                    }
                                    Thread.Sleep(300);
                                }
                            ...
                            {
                                YiWuNativeProvider.ic_exit(handle);
                            }
        ...
        private void StartAdjustCardType()
        {
            ...
            else
            {
                //临时屏蔽平台卡
                //StartReadAreaCard();
                ShowAlert(false,"读卡失败","未能识别您的卡片，请确认是否正确插入！",extend:new AlertExModel() {HideCallback = tp =>
                {
        ...
        //阅读社保卡
        protected virtual void StartReadInsuranceCard(string siCardNo)
        {
        ...
        protected virtual bool ProcessWithSiData(LoadingProcesser ctx,Res获取参保人员信息 res)
        {
            ...
            else
            {
                ...
                IdCardModel.Sex = dict.ContainsKey(patientInfo.性别) ? dict[patientInfo.性别] : (Convert.ToInt32(IdCardModel.IdCardNo[16]) % 2 == 0 ? Sex.女 : Sex.男);
                IdCardModel.Birthday = DateTime.ParseExact(patientInfo.出生日期, "yyyy-MM-dd", null, DateTimeStyles.None);
                Next();
                return true;
        ...
        protected Result<Res获取参保人员信息> GetInfoFromSi()
        {
            try
            {
                var cm = CardModel as CardModel;
                ...
                //}
                return Result<Res获取参保人员信息>.Fail(errMsg, new Exception(errMsg));
                // 
            }
            catch (Exception ex)
            {
        ...
        #region[密码输入框]
        protected bool _siPwdShowing = false;
        protected Uri _jiuZhenCard;
        protected Visibility _showSiCardAnimation;
        protected Visibility _showBarCodeCardAnimation;
        ...
        protected void ShowSiPasswordCallback()
        {
            if (_siPwdShowing)
            {
                return;
