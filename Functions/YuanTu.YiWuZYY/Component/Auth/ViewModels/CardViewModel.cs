...
using YuanTu.YiWuZYY.Component.Auth.Dialog;
...
namespace YuanTu.YiWuZYY.Component.Auth.ViewModels
{
    public class CardViewModel : YuanTu.Default.Component.Auth.ViewModels.CardViewModel
    {
        ...
        private void StartReadInsuranceCard(string siCardNo)
        {
            DoCommand(ctx =>
            {
                ...
                if (!ptInfoRest.IsSuccess)
                {
                    ...
                    ShowConfirm("读卡失败", "由于社保网络原因，系统将不能使用社保支付，请问继续操作吗?", cbk =>
                    {
                        if (cbk)
                        {
        ...
        public Visibility ShowHospitalCardButton => YiWuZyyConnst.IsEnable ? Visibility.Visible : Visibility.Collapsed;

        public Visibility ShowHospitalCardKeyboard => YiWuZyyConnst.CanScreenInput ? Visibility.Visible : Visibility.Collapsed;

        public string ShowHospitalCardButtonText => YiWuZyyConnst.ScreenInputText;
        //取消手动输入卡号
        ...
        public override void DoubleClick()
        {
            ...
            if (ret.IsSuccess)
            {
                ...
                DoCommand(lp =>
                {
                    ...
                    {
                        ShowAlert(false, "测试卡号","病人信息查询失败",debugInfo: PatientModel.Res病人信息查询.msg??"网关返回内容为空，请联系网关开发张强(188xxxxxxxxx)处理");
                    }
                });
            }
        }
