...
using YuanTu.YiWuZYY.Component.BillPay.Models;
using YuanTu.YiWuZYY.Models;
using CardModel = YuanTu.YiWuZYY.Models.CardModel;

namespace YuanTu.YiWuZYY.Component.BillPay.ViewModels
{
    public class BillRecordViewModel: YuanTu.Default.Component.BillPay.ViewModels.BillRecordViewModel
    {
        ...
        protected virtual void PerPayWithSi(LoadingProcesser lp)
        {
            ...
            PaymentModel.Self = (decimal.Parse(localBillPay.Res社保门诊预结算.计算结果信息.个人现金支付) * 100) ;
            PaymentModel.Insurance = decimal.Parse(localBillPay.Res社保门诊预结算.计算结果信息.合计报销金额) * 100;
            PaymentModel.Total = (decimal.Parse(localBillPay.Res社保门诊预结算.计算结果信息.费用总额) * 100);
            ...
            return DoCommand(lp =>
            {
                try
                {
                    ...
                    if (CardModel.CanUseInsurance())
                    {
                        ...
                        var ratiostr = string.Join("||", localBillPay.自负比例列表?.Select(p => p.ToString()) ?? new string[0]);
        ...
        protected virtual Result PayWithSi(LoadingProcesser lp)
        {
            ...
            {
                ShowAlert(false, "社保扣费失败", cmfres.错误信息);
                return Result.Fail(cmfres.错误信息);
            }
        ...
        protected override Queue<IPrintable> BillPayPrintables()
        {
            ...
            queue.Enqueue(new PrintItemText {Text = sb.ToString()});
            sb.Clear();
            var medicalFront = new Font("微软雅黑", 8, FontStyle.Regular);
            var ratio = new int[] {4, 3, 2, 2};
            queue.Enqueue(new PrintItemRatioText("名称","规格", "数量", "金额") {Font = medicalFront, TextSpaceRatios = ratio });
            if (record?.billItem != null)
            {
                foreach (var detail in record.billItem)
                {
                    queue.Enqueue(new PrintItemRatioText(detail.itemName, detail.itemSpecs, detail.itemQty, detail.billFee.InRMB()) {Font = medicalFront,TextSpaceRatios = ratio});
