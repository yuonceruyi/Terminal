...
using YuanTu.ShengZhouHospital.Component.BillPay.Services;
using YuanTu.ShengZhouHospital.HisNative.Models;

using BillPayModel = YuanTu.ShengZhouHospital.Component.BillPay.Models.BillPayModel;

namespace YuanTu.ShengZhouHospital.Component.BillPay.ViewModels
{
    public class BillRecordViewModel : YuanTu.Default.Component.BillPay.ViewModels.BillRecordViewModel
    {
        ...
        private void 凭条记录上传(Queue<IPrintable> data)
        {
            var req = new req凭条记录上传
            {
            ...
            var res = DataHandlerEx.凭条记录上传(req);
        }
        ...
        protected override Queue<IPrintable> BillPayPrintables()
        {
            ...
            if (dyd != null)
            {
                ...
                foreach (var key in dic.Keys)
                {
                    ...
                    {
                        var tmp = value[0].SureLength(8, true, true, ' ');
                        queue.Enqueue(new PrintItemRatioText($"{i++}." + tmp, value[1], value[2]) { TextSpaceRatios = new[] { 6, 2, 2 } });
                    }
                }
            }
            ...
            sb.Append($"1.如要需要发票请携带就诊卡和此凭条到人工窗口打印\n");
