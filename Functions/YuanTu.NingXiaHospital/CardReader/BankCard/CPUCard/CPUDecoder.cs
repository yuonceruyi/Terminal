...
namespace YuanTu.NingXiaHospital.CardReader.BankCard.CPUCard
{
    public class CPUDecoder
    {
        ...
        public static Dictionary<int, string> TagNames { get; set; } = new Dictionary<int, string>();
        ...
        public CPUDecoder Decode(byte[] data, Dictionary<int, CPUTlv> dictionary = null)
        {
            ...
            {
                if (!OmitLog)
                    Logger.Device.Info($"[银行卡读卡]{LogEntry}");
            }
            return this;
        }
        ...
        public static int ParseLengthNoExpand(ref int pos, ref byte[] data)
        {
            int len = data[pos];
            pos += 1;
            return len;
